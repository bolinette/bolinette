...
class Resource:
    def __init__(self, path: str, routes: list[Route] | None = None, /) -> None:
        self.path = path
        self.routes = {r.method: r for r in routes or []}

    def add(self, route: Route, /) -> None:
        if route.path != self.path:
            raise ValueError(f"Route path '{route.path}' does not match resource path '{self.path}'")
