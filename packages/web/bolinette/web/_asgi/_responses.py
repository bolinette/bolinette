...
from bolinette.web._abstract import ResponseState
from bolinette.web._asgi._types import HttpResponseResult, WebSocketSendResult
...
class AsgiSocketResponse:
    ...
    async def send(self, **kwargs: Any) -> None:
        if "raw" in kwargs:
            await self._send({"type": "websocket.send", "bytes": kwargs["raw"]})
        if "text" in kwargs:
            await self._send({"type": "websocket.send", "text": kwargs["text"]})
        if "json" in kwargs:
