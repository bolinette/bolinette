...
from bolinette.core import BolinetteContext, init
from bolinette.web import ext, WebContext, Controller, Cookie, Expects, Returns
from bolinette.data.defaults.services import UserService, RoleService
from bolinette.exceptions import ForbiddenError


@ext.controller('user', '/user')
class UserController(Controller):
    ...
    def _create_tokens(self, resp, user, *, set_access, set_refresh, fresh):
        ...
        if set_access:
            ...
            if cookies:
                resp.cookies.append(Cookie('access_token', access_token, http_only=True, path='/'))
        if set_refresh:
    ...
    @ext.route.get('/info', returns=Returns('user', 'private'), middlewares=['auth'])
    ...
    @ext.route.post('/login', expects=Expects('user', 'login'), returns=Returns('user', 'private'))
    ...
    async def logout(self):
        ...
        resp.cookies.append(Cookie('refresh_token', None, delete=True, path='/api/user/token/refresh'))
        return resp

    @ext.route.post('/token/refresh', middlewares=['auth|refresh'])
    ...
    @ext.route.post('/register', expects=Expects('user', 'register'), returns=Returns('user', 'private'))
    async def register(self, payload):
        ...
        """
        if init.get('ADMIN_REGISTER_ONLY', True):
    ...
    @ext.route.post('/register/admin', expects=Expects('user', 'admin_register'),
                    returns=Returns('user', 'private'), middlewares=['auth|roles=admin'])
    ...
    @ext.route.patch('/me', expects=Expects('user', 'register', patch=True), returns=Returns('user', 'private'),
    ...
    @ext.route.post('/{username}/roles', expects=Expects('role'), returns=Returns('user', 'private'),
    ...
    @ext.route.delete('/{username}/roles/{role}', returns=Returns('user', 'private'), middlewares=['auth|roles=admin'])
    ...
    @ext.route.post('/picture', returns=Returns('user', 'private'), middlewares=['auth'])
