...
from bolinette.data import (
    DataContext, Mixin, MixinMetadata, MixinServiceMethod,
    Service, SimpleService, ServiceMetadata, Seeder
)
...
class DataExtension(BolinetteExtension[DataContext]):
    ...
    def service(self, service_name: str, *, model_name: str = None):
        def decorator(service_cls: type[Service | SimpleService]):
            service_cls.__blnt__ = ServiceMetadata(service_name, model_name or service_name)
...
class _MixinDecorator:
    ...
    def __call__(self, mixin_name: str):
        def decorator(mixin_cls: type[Mixin]):
...
class _ModelDecorator:
    ...
    def __call__(self,
        ...
        def decorator(model_cls: type[Model]):
            model_cls.__blnt__ = ModelMetadata(model_name, database, model_type == 'relational',
